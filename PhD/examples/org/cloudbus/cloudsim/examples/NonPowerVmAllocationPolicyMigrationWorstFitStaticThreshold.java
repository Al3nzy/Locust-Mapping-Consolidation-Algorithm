 ...
 * {@link #getUtilizationThreshold() over} utilization. It selects the host to
 * place a VM, that one having the most remaining CPU which is enough to attend
 * the VM (Worst Fit policy), <b>disregarding energy consumption</b>.
 *
 * @author Manoel Campos da Silva Filho
 */
public class NonPowerVmAllocationPolicyMigrationWorstFitStaticThreshold
	...
	public NonPowerVmAllocationPolicyMigrationWorstFitStaticThreshold(List<? extends Host> hostList,
	...
	/**
	 * Gets the first PM that has enough resources to host a given VM, which has the
	 * most available capacity and will not be overloaded after the placement.
	 * 
	...
	public PowerHost findHostForVm(Vm vm, Set<? extends Host> excludedHosts) {
		for (PowerHost host : this.<PowerHost>getHostList()) {
			if (!excludedHosts.contains(host) && host.isSuitableForVm(vm)
				...
				// vm.getCurrentRequestedTotalMips: ", vm.getCurrentRequestedTotalMips(),"
				// host.getTotalMips: ", host.getTotalMips());
				return host;
			}
		}
	...
	protected PowerHost getUnderUtilizedHost(Set<? extends Host> excludedHosts) {
		...
		for (PowerHost host : this.<PowerHost>getHostList()) {
			...
			if (utilization > 0 && utilization < underUtilizationThreshold && utilization < minUtilization
	...
	/**
	 * Gets the percentage of total CPU utilization to indicate that a host is under
	 * used and its VMs have to be migrated.
	 * 
	 * @return the under utilization threshold (in scale is from 0 to 1, where 1 is
	 *         100%)
	 */
	...
	/**
	 * Sets the percentage of total CPU utilization to indicate that a host is under
	 * used and its VMs have to be migrated.
	 * 
	 * @param underUtilizationThreshold the under utilization threshold (in scale is
	 ...
	 * their availability of CPU MIPS, considering the capacity of all Host PEs.
	 * This Comparator is used to allow sorting a list of VMs in methods such as
	 * {@link #getHostList()}.
	 *
	...
	class PowerHostComparator implements Comparator<PowerHost> {
		...
		/**
		 * Compares two hosts in an operation simulator to {@code host1 &gt; host2}. The
		 * host with the most available CPU MIPS is considered as greater than the
		 * other. Thus, in a sort operation, the host will be sorted in increasing order
		 * of available CPU MIPS.
		 *
