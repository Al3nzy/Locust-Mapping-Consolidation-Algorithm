 ...
 * 
 * To select the minimum VM utilization to migrate it (based on the cloudlets that running inside and their utilization). (alanzy)
 *  
 * 
...
public class PowerVmSelectionPolicyMinimumUtilization extends PowerVmSelectionPolicy {
	...
	public Vm getVmToMigrate(PowerHost host) {
		...
		for (Vm vm : migratableVms) {
			...
			double getting = vm.getTotalUtilizationOfCpuMips(CloudSim.clock());
			if (metric < minMetric) {
				minMetric = metric;
				vmToMigrate = vm;
