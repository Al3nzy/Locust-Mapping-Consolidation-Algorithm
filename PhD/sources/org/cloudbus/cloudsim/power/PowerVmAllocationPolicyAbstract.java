...
public abstract class PowerVmAllocationPolicyAbstract extends VmAllocationPolicy {
	...
	/*
	 * (non-Javadoc)
	 * ( Alanzy )in this function the first VM come will be allocated to the first suitable Host (i.e. after checking the PE capacity, RAM, BW, available MIPS of the host ).
	...
	public boolean allocateHostForVm(Vm vm, Host host) {
		...
		if (host.vmCreate(vm)) { // if vm has been successfully created in the host
			...
			Log.formatLine("%.2f: (VM #" + vm.getId() +" MIPS: "+vm.getMips()+ ")  has been allocated to the (host #" + host.getId()+" MIPS: "+host.getTotalMips()/host.getNumberOfPes()+")",
					CloudSim.clock());
			return true;
		}
	 ...
	 * 
	 * @see
	 * org.cloudbus.cloudsim.VmAllocationPolicy#getHost(org.cloudbus.cloudsim.Vm)
	 */
	@Override
