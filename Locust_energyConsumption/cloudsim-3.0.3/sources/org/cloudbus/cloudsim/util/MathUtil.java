...
public class MathUtil {
	...
	public static SimpleRegression createWeigthedLinearRegression(final double[] x, final double[] y,
		...
		for (int i = 0; i < x.length; i++) {
			if (numZeroWeigths >= 0.4 * weigths.length) {
				// See:
				// http://www.ncsu.edu/crsc/events/ugw07/Presentations/Crooks_Qiao/Crooks_Qiao_Alt_Presentation.pdf
	...
	public static double[] getRobustLoessParameterEstimates(final double[] y) {
		...
		SimpleRegression tricubeBySquareRegression = createWeigthedLinearRegression(x, y,
				getTricubeBisquareWeigts(residuals));

		double[] estimates = tricubeBySquareRegression.regress().getParameterEstimates();
