...
public class HarddriveStorage implements Storage {
	...
	/**
	 * Creates a new harddrive storage with a given capacity. In this case the name
	 * of the storage is a default name.
	 * 
	 * @param capacity the capacity in MByte
	 ...
	 * parameters, such as latency, average seek time and maximum transfer rate are
	 * set. The default values are set to simulate the Maxtor DiamonMax 10 ATA
	 * harddisk. Furthermore, the necessary lists are created.
	 */
	private void init() {
	...
	/**
	 * Get the seek time for a file with the defined size. Given a file size in MB,
	 * this method returns a seek time for the file in seconds.
	 * 
	...
	/**
	 * Check if the file is valid or not. This method checks whether the given file
	 * or the file name of the file is valid. The method name parameter is used for
	 * debugging purposes, to output in which method an error has occured.
	 * 
	 * @param file       the file to be checked for validity
	 * @param methodName the name of the method in which we check for validity of
	 *                   the file
	...
	/**
	 * Adds a file to the storage. First, the method checks if there is enough space
	 * on the storage, then it checks if the file with the same name is already
	 * taken to avoid duplicate filenames. <br>
	...
	/**
	 * Adds a set of files to the storage. Runs through the list of files and save
	 * all of them. The time taken (in seconds) for adding each file can also be
	 ...
	 * {@link gridsim.datagrid.File#getTransactionTime()}.
	 * 
	 * @param file the file which is removed from the storage is returned through
	 *             this parameter
	 * @return the time taken (in seconds) for deleting the specified file
	 */
	@Override
