...
public abstract class PowerVmAllocationPolicyMigrationAbstract extends PowerVmAllocationPolicyAbstract {
	...
	public List<Map<String, Object>> optimizeAllocation(List<? extends Vm> vmList) {
		...
		List<? extends Vm> vmsToMigrate = getVmsToMigrateFromHosts(overUtilizedHosts); // select all VM in the over-utilized host as a VM to be migrated (Alanzy)
	...
	protected List<Map<String, Object>> getMigrationMapFromUnderUtilizedHosts(
		...
		while (true) {
			if (numberOfHosts == excludedHostsForFindingUnderUtilizedHost.size()) {
				//Log.printLine("All Hosts are over-utilized");
				break;
			}
			...
			Log.print("Reallocation of VMs from the under-utilized host#"+underUtilizedHost.getId()+ " (VM#(|" );
			if (!Log.isDisabled()) {
	...
	//	int i=0;
		...
		//	System.out.println("Host ID: "+i );
			if (excludedHosts.contains(host)) {
				//System.out.println("I am in excluded host loop :"+i );
				continue;
			}
			if (host.isSuitableForVm(vm)) {
				//System.out.println("I am in isSuitableForVm (Host ID:"+i );
				if (getUtilizationOfCpuMips(host) != 0 && isHostOverUtilizedAfterAllocation(host, vm)) {
				//we can here add code to submit for FCFS submission algorithm for VM allocation (Alanzy) then adding and brake code to exit form this searching loop
					continue;
				}
				//int j=0;
			//	System.out.println("HOST ID:::: "+i+" and J:"+j);
				try {
					//System.out.println("HOST ID:::: "+i+" I am in try now " );
					double powerAfterAllocation = getPowerAfterAllocation(host, vm);
					if (powerAfterAllocation != -1) {
						//( Alanzy ) the number of the PE will have the main role here (e.g. if PE set is (2,4,6) so it will select firstly from they 2 if satisfied then from PE=4 and so on)
						double powerDiff = powerAfterAllocation - host.getPower();
					//	System.out.println("    powerDiff: "+powerDiff +"     Power after allocation: "+powerAfterAllocation+"     Host power: "+ host.getPower() );
