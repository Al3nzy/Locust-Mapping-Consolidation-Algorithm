...
/**
 * An Cloudlet {@link UtilizationModel} that uses Arithmetic Progression to
 * increases the utilization of the related resource along the simulation time.
 * 
...
public class UtilizationModelArithmeticProgression implements UtilizationModel {
	...
	public UtilizationModelArithmeticProgression(final int cloudletId,
	...
	/**
	 * Instantiates a UtilizationModelProgressive that sets the
	 * {@link #setUtilizationPercentageIncrementPerSecond(double) utilization
	 * increment} and the {@link #setInitialUtilization(double) initial utilization}
	 * 
	 * @param cloudletId
	 ...
	 */
	public UtilizationModelArithmeticProgression(final int cloudletId,
			final double utilizationPercentageIncrementPerSecond, final double initialUtilization) {
	...
	/**
	 * Gets the utilization percentage to be incremented at the total utilization
	 * returned by {@link #getUtilization(double)} at every simulation second.
	 * 
	...
	/**
	 * Sets the utilization percentage to be incremented at the total utilization
	 * returned by {@link #getUtilization(double)} at every simulation second.
	 * 
	 * @param utilizationPercentageIncrementPerSecond the utilization percentage
	 *                                                increment to be set. For
	 *                                                instance, if given the value
	 *                                                0.1, it means that every
	 *                                                simulation second, the total
	 *                                                utilization will be
	 *                                                incremented by 1%. If set a
	 *                                                negative value, the
	 *                                                utilization will be decreased
	 *                                                every second by the given
	 *                                                percentage. If the value is
	 *                                                set as 0, it means the
	 *                                                utilization will be equals o
	 *                                                the
	 *                                                {@link #getInitialUtilization()
	 *                                                initial utilization} all the
	 *                                                time.
	 */
	@SuppressWarnings("unused")
	...
	/**
	 * Gets the initial percentage of resource that cloudlets using this
	 * UtilizationModel will require when they start to execute.
	 * 
	 * @return the initial utilization percentage (in scale is from 0 to 1, where 1
	 *         is 100%)
	 */
	...
	/**
	 * Sets the initial percentage of resource that cloudlets using this
	 * UtilizationModel will require when they start to execute.
	 * 
	 * @param the initial utilization percentage (in scale is from 0 to 1, where 1
	 *            is 100%)
	 */
	private void setInitialUtilization(double initialUtilization) {
		if (initialUtilization < 0 || initialUtilization > 1)
			throw new RuntimeException(
					"Initial utilization must to be a percentage value between [0 and 1] (0 to 100%)");
	 ...
	 * 
	 * @return the maximum resource usage percentage (in scale from [0 to 1], where
	 *         1 is equals 100%)
	 */
	 ...
	 * 
	 * @param maxResourceUsagePercentage the maximum resource usage percentage (in
	 *                                   scale from [0 to 1], where 1 is equals
	 *                                   100%)
	 */
